...
from ..core import state, is_retryable_error, stats_manager, apply_model_routing
    ...
    for i, item in enumerate(input_data):
        ...
        if item_type == "message":
            ...
            if role == "user":
                ...
                text = strip_thinking_from_text(text) if text else ""
...
async def handle_responses(request: Request):
    ...
    model_raw = apply_model_routing(body.get("model", "gpt-4o"))
    ...
    thinking_cfg, thinking_explicit = extract_thinking_config_from_openai_body(body)
    if not thinking_explicit and infer_thinking_from_openai_responses_input(input_data):
    ...
    if stream:
        return await _handle_stream(kiro_request, headers, account, model, log_id, start_time, thinking_cfg)
    
    # 非流式 - kiro.rs 风格：单阶段处理
    if thinking_cfg.enabled:
        # 使用思维链配置构建请求
        main_user_content = build_thinking_prompt(
    ...
    async with httpx.AsyncClient(verify=False, timeout=120) as client:
        ...
        return _build_response(result, model, log_id, thinking_cfg.enabled)


def _build_response(result: dict, model: str, response_id: str, thinking_enabled: bool = False) -> dict:
    """构建非流式响应 - kiro.rs 风格：单阶段处理"""
    text = "".join(result.get("content", []))
    # kiro.rs 风格：如果启用思维链，从响应中移除思维链标签
    if thinking_enabled:
...
async def _handle_stream(kiro_request, headers, account, model, log_id, start_time, thinking_cfg: ThinkingConfig):
    ...
    async def generate():
        ...
        # kiro.rs 风格：单阶段处理，直接修改请求内容
        if thinking_cfg.enabled:
            try:
                cs = kiro_request.get("conversationState", {})
                cm = cs.get("currentMessage", {}).get("userInputMessage", {})
                original_user = cm.get("content", "")

                # 使用思维链配置构建请求内容
                main_user = build_thinking_prompt(
                    user_content=original_user,
                    budget_tokens=thinking_cfg.budget_tokens,
                    history=cs.get("history", []),
                    has_tool_results=bool(cm.get("userInputMessageContext", {}).get("toolResults"))
                )
                if "content" in cm:
                    cm["content"] = main_user
            except Exception:
                pass
        ...
        try:
            async with httpx.AsyncClient(verify=False, timeout=300) as client:
                async with client.stream("POST", KIRO_API_URL, json=kiro_request, headers=headers) as response:
                    ...
                    # 3. 流式读取并发送 delta
                    full_response = b""
                    content_buffer = ""  # 用于处理跨chunk的thinking标签
                            ...
                            if thinking_cfg.enabled:
                                ...
                                while i < len(content_buffer):
                                    if not in_thinking_block:
                                        # 查找thinking开始标签
                                        thinking_start = content_buffer.find("<thinking>", i)
                                        if thinking_start == -1:
                                            # 没有找到thinking标签，输出剩余内容
                                            output_content += content_buffer[i:]
                                            content_buffer = ""
                                            break
                                        else:
                                            # 输出thinking标签之前的内容
                                            output_content += content_buffer[i:thinking_start]
                                            in_thinking_block = True
                                            i = thinking_start + len("<thinking>")
                                    else:
                                        # 在thinking块中，查找结束标签
                                        thinking_end = content_buffer.find("</thinking>", i)
                                        if thinking_end == -1:
                                            # 没有找到结束标签，保留缓冲区内容等待更多数据
                                            content_buffer = content_buffer[i:]
                                            break
                                        else:
                                            # 找到结束标签，跳过thinking块
                                            in_thinking_block = False
                                            i = thinking_end + len("</thinking>")

                                content = output_content

                            if content:  # 只有在过滤后还有内容时才发送
                    ...
                    if not full_content:
                        ...
                        # 确保完整内容也被过滤
                        if thinking_cfg.enabled:
                            full_content = strip_thinking_from_text(full_content)
