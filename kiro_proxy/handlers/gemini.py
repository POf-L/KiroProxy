...
from ..core import state, is_retryable_error, apply_model_routing
...
async def handle_generate_content(model_name: str, request: Request):
    ...
    thinking_cfg, thinking_explicit = extract_thinking_config_from_gemini_body(body)
    # 移除自动推断逻辑 - 只有用户明确启用时才使用思维链
    # if not thinking_explicit and infer_thinking_from_gemini_contents(contents):
    #     thinking_cfg = ThinkingConfig(True, None)
    
    model_raw = apply_model_routing(model_name.replace("models/", ""))
            ...
            async with httpx.AsyncClient(verify=False, timeout=120) as client:
                ...
                if resp.status_code == 429:
                    ...
                    if next_account and retry < max_retries:
                        print(f"[Gemini] 429 限流，切换账号: {current_account.id} -> {next_account.id}")
                ...
                if resp.status_code != 200:
                    ...
                    if error.type == ErrorType.CONTENT_TOO_LONG:
                        ...
                        if should_retry:
                            ...
                            print(f"[Gemini] 内容长度超限但未重试: retry={retry}/{max_retries}{reason}")
    ...
    # 使用转换函数生成 Gemini 格式响应 - kiro.rs 风格：内容已包含思维链
    if thinking_cfg.enabled:
        result = dict(result)
        clean_text = strip_thinking_from_text("".join(result.get("content", []) or []))
